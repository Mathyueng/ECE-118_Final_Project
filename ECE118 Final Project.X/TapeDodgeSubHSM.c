 ...
 * 10/30/11 17:59 jec      fixed references to CurrentEvent in RunTapeDodgeSM()
...
static TapeDodgeSubHSMState_t CurrentState = InitPSubState; // <- change name to match ENUM
...
/**
 * @Function InitTapeDodgeSubHSM(uint8_t Priority)
...
uint8_t InitTapeDodgeSubHSM(void) {
    ES_Event returnEvent;

    CurrentState = InitPSubState;
    returnEvent = RunTapeDodgeSubHSM(INIT_EVENT);
...
ES_Event RunTapeDodgeSubHSM(ES_Event ThisEvent) {
    uint8_t makeTransition = FALSE; // use to flag transition
    TapeDodgeSubHSMState_t nextState; // <- change type to correct enum
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunTapeDodgeSubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunTapeDodgeSubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
#ifdef LED_USE_DODGE
